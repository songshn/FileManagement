...
using DHJ.FileManagement.Tests.TestDatas;
...
namespace DHJ.FileManagement.Tests
{
    ...
    /// Provides methods to easily work with <see cref="FileManagementDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<FileManagementTestModule>
    {
        ...
        {
            void NormalizeDbContext(FileManagementDbContext context)
            {
        ...
        protected void UsingDbContext(Action<FileManagementDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FileManagementDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FileManagementDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FileManagementDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FileManagementDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FileManagementDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FileManagementDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FileManagementDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FileManagementDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FileManagementDbContext, Task<T>> func)
        {
            T result;
